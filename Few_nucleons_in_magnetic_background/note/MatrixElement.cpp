...
for (int ipair = 0; ipair < nPairs; ipair++)   //sum over pairs
	{
      ...
      MagneticEnergy = MagneticEnergy + parity[iperm] * stme[iperm*nPairs*nop]*x*sy;
      harmonic = harmonic + parity[iperm] * stme[iperm*nPairs*nop] *x* (sx+sy+sz);
    } 
...
//======2-body===============================================
      for (int ipair = 0; ipair < nPairs; ipair++)   //sum over pairs
	{
      ...
      MagneticEnergy = pow(npar, -1)*0.5 * h2m * eB * eB * MagneticEnergy; 
      harmonic = pow(npar, -1)*0.5 * h2m * momega * momega * harmonic; 
      if(MS==0) MagneticSpin = 0;
      else MagneticSpin = -1*h2m * eB * MagneticSpin;
