...
int sign(double x);
double SpinOp(std::vector<int> sz1, std::vector<int> sz2, int npar, int ipar, VectorXi Perm, int i_spin_coupling);
double IsospinOp(std::vector<int> tz1, std::vector<int> tz2, int npar, int ipar, VectorXi Perm, int i_spin_coupling);

double SpinOp_pair(std::vector<int> sz1, std::vector<int> sz2, int npar, int i, int j, VectorXi Perm);
double IsospinOp_pair(std::vector<int> tz1, std::vector<int> tz2, int npar, int i, int j, VectorXi Perm, int index);
...
double MatrixElement::energy(std::vector<MatrixXd> state1, std::vector<MatrixXd> state2)
{
	...
	double PotEnergy = 0, KinEnergy = 0, MagneticEnergy=0, MagneticEnergyT=0,MagneticSpin=0, harmonic=0, PotEnergy3B = 0, PotEnergy3BP = 0;
	...
	for (int iperm = 0; iperm < nPerm; iperm++)   //sum over permutation
    {
    	A2x = PM[iperm].transpose() * A2_0x * PM[iperm];
      	...
      	{
      		MagneticEnergyT = MagneticEnergyT + parity[iperm] * magnetic_charge_me[iperm*npar+ipar] * (InvAAx(ipar,ipar)+InvAAy(ipar,ipar)) * x;
      	}
		//==================================================================================
		...
		for (int i = 0; i < npar; i++)
		   {
		       for (int j = i+1; j < npar; j++)
		          {
		           sx = magnetic_me_pair[iperm*nPairs+ipair](0) * InvAAx(i,i) + magnetic_me_pair[iperm*nPairs+ipair](1)*InvAAx(j,j)-2*magnetic_me_pair[iperm*nPairs+ipair](2)*InvAAx(i,j);
		           sy = magnetic_me_pair[iperm*nPairs+ipair](0) * InvAAy(i,i) + magnetic_me_pair[iperm*nPairs+ipair](1)*InvAAy(j,j)-2*magnetic_me_pair[iperm*nPairs+ipair](2)*InvAAy(i,j);
		           sz = magnetic_me_pair[iperm*nPairs+ipair](0) * InvAAz(i,i) + magnetic_me_pair[iperm*nPairs+ipair](1)*InvAAz(j,j)-2*magnetic_me_pair[iperm*nPairs+ipair](2)*InvAAz(i,j);
		           ipair++;
	...
	for (int icmp = 0; icmp < isospin.ncmp; icmp++){
	  	for (int jcmp = 0; jcmp < isospin.ncmp; jcmp++){
	    	y = y + isospin.coef[icmp] * isospin.coef[jcmp]
                * IsospinOp(isospin.cmp[icmp].tz, isospin.cmp[jcmp].tz, npar, ipar, Perm, i_spin_coupling);
	  	}
	}
	return x*y;
}
double SpinOp(std::vector<int> sz1, std::vector<int> sz2, int npar, int ipar, VectorXi Perm, int i_spin_coupling)
{
	for (int ip = 0; ip < npar; ip++)	if (sz1[ip] != sz2[Perm(ip)]) return 0;
...
double IsospinOp(std::vector<int> tz1, std::vector<int> tz2, int npar, int ipar, VectorXi Perm, int i_spin_coupling)
{
	for (int ip = 0; ip < npar; ip++)	if (tz1[ip] != tz2[Perm(ip)]) return 0;
	if(i_spin_coupling==1)
 	{
