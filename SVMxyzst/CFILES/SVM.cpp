...
/*  see docu in PRC 52 6 (1995) K.Varga and Y.Suzuki
    NewState calls =>
    Basis = old Basis + 1 new random State;
    C,D   = H eigenvectors,values in old basis
    E     = Min(D)
    EE    = E + abs(E / 2)    if itr == 1
*/
double SVM::NewEnergy(vector<BasisState> &Basis, MatrixXd C, VectorXd D, double E, double EE)
{
	...
	// eq.(12) numerator, 2nd term <EV(1-k)|EV(k+1)>
    ...
    // eq.(12) numerator, 2nd term  -|EV(1-k)> <EV(1-k)|EV(k+1)>
	c(itr) = 1;
	for (int k1 = 0; k1 < itr; k1++)
	{
		for (int k2 = 0; k2 < itr; k2++)
	...
	double e3 = E;
	//  D: Eigenvalues in old basis q,a as def. in eq.(14)
...
BasisState SVM::FirstNewState(){
    ...
    /* try 10 times to randomly generate a state with non-zero norm*/
    while (count<10){
	    count++;
	    int ts=ChooseSTstate();
    ...
    /* randomly generate 1000 states with non-zero norm and compare their <H>
       with the previously obtained, keep the state only if E_new < E_old  */
    double NewE;
    BasisState State;
    State = NewState;
    count = 0;
...
BasisState SVM::NewState(vector<BasisState> &Basis, MatrixXd C, VectorXd D, double E, double EE)
{
    ...
    /*add a random state to the basis*/
    BasisState NewState;
	...
	while(count4<=mm0){
	    ...
	    /* the new state either produced an unstable norm or did not lower the
	       energy noticably */
        if(xx==0){
            count4++;
            dx = Dmatrix();
        ...
        /* the newly added state decreased the energy by an amount larger than
           a defined threshold */
        if(xx==1) break;
    }

	/* return a new state if the lowest eigenvalue of the old basis with this
	   state is lower that a predefined amount compared with the old one */
	return State;
}
//=============================================================================
