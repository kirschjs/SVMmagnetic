...
double MatrixElement::energy(std::vector<MatrixXd> state1, std::vector<MatrixXd> state2)
{
	...
	for (int iperm = 0; iperm < nPerm; iperm++)   //sum over permutation
	//for (int iperm = 0; iperm < 1; iperm++)   //sum over permutation
    {
	    A2x = PM[iperm].transpose() * A2_0x * PM[iperm];
	    A2y = PM[iperm].transpose() * A2_0y * PM[iperm];
	    A2z = PM[iperm].transpose() * A2_0z * PM[iperm];
		...
		x=1/sqrt(detx); // 1-dimensional
	    //x=1/sqrt(detx*dety*detz); // 3-dimensional
	    
		// Kinetic Energy 
	    TTx = A1x * InvAAx * A2x;
	    TTy = A1y * InvAAy * A2y;
	    TTz = A1z * InvAAz * A2z;
	    KinEnergy = KinEnergy + parity[iperm]  *TTx.trace()*x;                             // 1-dimensional
		//KinEnergy = KinEnergy + parity[iperm] * stme[iperm*nPairs*nop] *(TTx.trace()+TTy.trace())*x;             // 2-dimensional
		//KinEnergy = KinEnergy + parity[iperm] * stme[iperm*nPairs*nop] *(TTx.trace()+TTy.trace()+TTz.trace())*x; // 3-dimensional
		
		double sx,sy,sz, xme;

		//=============MagneticEnergy====================================     
		harmonic = harmonic + parity[iperm]  * 
		x * InvAAx.trace();                                 // 1-dimensional
		//x * (InvAAx.trace()+InvAAz.trace());                // 2-dimensional
		//x * stme[iperm*nPairs*nop] * (InvAAx.trace()+InvAAy.trace()+InvAAz.trace()); // 3-dimensional
		
		//double testTb = (double)(InvAAx(0,1));
		//double testTc = (double)(InvAAx(1,0));
		//double testTd = (double)(InvAAx(1,1));
    ...
    MagneticEnergy = eB * MagneticEnergy; 
    //harmonic       = pow(npar, -2)* 4 * h2m * momega * momega * harmonic;
	harmonic       = 0.5 * pow(h2m, -1) * momega * momega * harmonic;

//  	return PotEnergy+KinEnergy+MagneticEnergy+harmonic+PotEnergy3B;
  	return KinEnergy+harmonic;
}
