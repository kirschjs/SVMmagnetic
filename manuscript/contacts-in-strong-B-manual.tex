\documentclass[aps,prd,onecolumn
,tightenlines,letterpaper,notitlepage,
%superscriptaddress,
nofootinbib]{revtex4-1}
\usepackage{amssymb,latexsym}
\usepackage{amsmath,amsbsy,bbm}
\usepackage{epsfig,bm,color}
\usepackage{cjhebrew}
\usepackage{nicefrac}
\usepackage{graphicx,comment}
\usepackage{slashed}
\usepackage{bbold}
\usepackage{lipsum,pdflscape}
\usepackage{tikz-feynman}

%\usepackage{hyperref}
\unitlength=1mm
\DeclareMathOperator{\st}{str}
\DeclareMathOperator{\Erfc}{Erfc}
\DeclareMathOperator{\Erf}{Erf}
\DeclareMathOperator{\Pf}{Pf}
\DeclareMathOperator{\sign}{sign}

\usepackage{dutchcal}

\usepackage{calligra}

\DeclareMathAlphabet{\mathcalligra}{T1}{calligra}{m}{n}
\DeclareFontShape{T1}{calligra}{m}{n}{<->s*[2.2]callig15}{}
\newcommand{\scriptr}{\mathcalligra{r}\,}
\newcommand{\boldscriptr}{\pmb{\mathcalligra{r}}\,}
\definecolor{blue}{HTML}{4169E1}
\definecolor{red}{HTML}{DC143C}
\definecolor{green}{HTML}{2E8B57}
\definecolor{black}{HTML}{000000}
\newcommand{\black}[1]{\textcolor{black}{#1}}
\newcommand{\red}[1]{\textcolor{red}{#1}}
\newcommand{\blue}[1]{\textcolor{blue}{#1}}
\newcommand{\green}[1]{\textcolor{green}{#1}}

\begin{document}

\def\a{{\alpha}}
\def\b{{\beta}}
\def\d{{\delta}}
\def\D{{\Delta}}
\def\X{{\Xi}}
\def\e{{\varepsilon}}
\def\g{{\gamma}}
\def\G{{\Gamma}}
\def\k{{\kappa}}
\def\l{{\lambda}}
\def\L{{\Lambda}}
\def\m{{\mu}}
\def\n{{\nu}}
\def\o{{\omega}}
\def\O{{\Omega}}
\def\S{{\Sigma}}
\def\s{{\sigma}}
\def\th{{\theta}}

\def\ol#1{{\overline{#1}}}

\def\Dslash{D\hskip-0.65em /}
\def\Dtslash{\tilde{D} \hskip-0.65em /}

\def\order{{\mathcal O}}

\def\Dt{{\tilde{D}}}
\def\St{{\tilde{\Sigma}}}

\def\eqref#1{{(\ref{#1})}}

\newcommand{\caf}{\text{\cjRL{b}}}
\newcommand{\he}{${}^4$He}
\newcommand{\hes}{${}^3$He}
\newcommand{\tr}{${}^3$H}
\newcommand{\ls}{\ve{L}\cdot\ve{S}}
\newcommand{\eps}{\epsilon}
\newcommand{\as}{a_s}
\newcommand{\at}{a_t}
\newcommand{\ecm}{E_\textrm{\small c.m.}}
\newcommand{\dq}{\mbox{d\hspace{-.55em}$^-$}}
\newcommand{\mpis}{$m_\pi=137~${\small MeV}}
\newcommand{\mpim}{$m_\pi=450~${\small MeV}}
\newcommand{\mpil}{$m_\pi=806~${\small MeV}}
\newcommand{\muh}{\mu_{^3\text{\scriptsize He}}}
\newcommand{\mut}{\mu_{^3\text{\scriptsize H}}}
\newcommand{\mud}{\mu_\text{\scriptsize D}}
\newcommand{\pode}{\beta_{\text{\scriptsize D},\pm1}}
\newcommand{\poh}{\beta_{^3\text{\scriptsize He}}}
\newcommand{\pot}{\beta_{^3\text{\scriptsize H}}}
\newcommand{\com}[1]{{\scriptsize \sffamily \bfseries \color{red}{#1}}}
\newcommand{\eg}{\textit{e.g.}\;}
\newcommand{\ie}{\textit{i.e.}\;}
\newcommand{\cf}{\textit{c.f.}\;}
\newcommand{\be}{\begin{equation}}
\newcommand{\ee}{\end{equation}}
\newcommand{\la}{\label}
\newcommand{\bs}[1]{\ensuremath{\boldsymbol{#1}}}
\newcommand{\bk}{\bs k}
\newcommand{\bt}{B_{^{3}\text{H}}}
\newcommand{\bh}{B_{^{3}\text{He}}}
\newcommand{\bd}{B_\text{D}}
\newcommand{\ba}{B_\alpha}
\newcommand{\rgm}{$\mathbb{R}$GM}
\newcommand{\bra}[1] {\left\langle~#1~\right|}
\newcommand{\ket}[1] {\left|~#1~\right\rangle}
\newcommand{\overlap}[2] {\left\langle~#1~\left|~#2~\right.\right\rangle}
\newcommand{\me}[3] {\left\langle~#1~\left|~#2~\left|~#3~\right.\right.\right\rangle}
\newcommand{\lam}[1]{$\Lambda=#1~$fm$^{-1}$}
\newcommand{\nopi}{\pi\hspace{-6pt}/}
\newcommand{\ve}[1]{\ensuremath{\boldsymbol{#1}}}
\newcommand{\sumin}{\sum\limits_{i=1}^\texttt{N}}
\newcommand{\sumijn}{\sum\limits_{i<j}^\texttt{N}}
\newcommand{\sumijk}{\sum\limits_\text{cyc.}\sum\limits_{i<j<k}}
\newcommand{\prodcart}{\prod\limits_{c=x,y,z}}
\newcommand{\tx}{\tilde{x}}
\newcommand{\eftnopi}{\mbox{EFT($\slashed{\pi}$)}}
\newcommand{\pe}[1]{\ensuremath{\mathfrak{P}_{#1}}}
\newcommand{\cmment}[2]{\paragraph*{Ecce: #1}\texttt{\textcolor{blue}{#2}}}



\author{$\mathcal{M}$.~$\mathcal{Elyahu}$}
\author{$\mathcal{N}$.~$\mathcal{Barnea}$}
\author{$\mathcal{J}$.~$\mathcal{Kirscher}$}
%\email[]{$\texttt{bctiburz@chinamail.cn}$}
%\affiliation{%
%$\mathcal{CCNY}$ (Consolidated Chinese National Yeshiva)}

\title{
Strong magnetic fields and contact interactions in few-fermion systems
} 

\begin{abstract}
Technical manual detailing the implementation of a variational solution of the non-relativistic few-body
problem in an external, \ie, static magnetic field.
\end{abstract}

\pacs{}

\maketitle

\begin{landscape}
\paragraph{The symmetric Gauge}
\be\la{eq.symmgauge}
\ve{A}_i=\frac{B_0}{2}(-y(i),x(i),0)
\ee

\paragraph{The Hamiltonian}
\begin{align}\la{eq.ham}
\hat{H}=-\frac{\hbar^2}{2m}&\sumin\Bigg\lbrace
\ve{\nabla}(i)^2
+i\left(\frac{\hbar^2}{2m}\right)\left(\frac{q(i)B_0}{\hbar}\right)
L_z(i)
+\left(\frac{\hbar^2}{2m}\right)\left(\frac{q(i)B_0}{\hbar}\right)^2\frac{1}{4}
\left(x^2(i)+y^2(i)\right)
-g(i)\left(\frac{\hbar^2}{2m}\right)\left(\frac{q(i)B_0}{\hbar}\right)\sigma_z(i)
\Bigg\rbrace\\
&+~\sumijn\left(C_a +
C_b\left[\sigma^+(i)\sigma^-(j)+\sigma^-(i)\sigma^+(j)-\sigma^z(i)\sigma^z(j)\right]\right)~e^{-\frac{\Lambda^2}{4}\left[\ve{r}(i)-\ve{r}(j)\right]^2}
+\sumijk D\cdot
e^{-\frac{\Lambda^2}{4}
\left[\ve{r}(i)-\ve{r}(j)\right]^2}
e^{-\frac{\Lambda^2}{4}\left[\ve{r}(i)-\ve{r}(k)\right]^2}
\end{align}

\paragraph{The variational basis}
\be\la{eq.svmME}
\ket{A,\alpha}:=
e^{-\frac{1}{2}\ve{x}^TA_x\ve{x}}~
e^{-\frac{1}{2}\ve{y}^TA_y\ve{y}}~
e^{-\frac{1}{2}\ve{z}^TA_z\ve{z}}\cdot
\sum\limits_{n=1}^{\texttt{ncmp}}C^n_\alpha\ket{s^n_1,\ldots,s^n_\texttt{N}~;~t^n_1,\ldots,t^n_\texttt{N}}
\ee
with
\[\begin{array}{c}
\alpha=1,~\ldots,~\texttt{nts\_states}\\
\end{array}\]

\paragraph{The generic matrix element}
\be\la{eq.genericme}
I_\mathcal{O}(A',\alpha',A,\alpha;P):=
\me{A',\alpha'}{\hat{\mathcal{O}}\otimes\hat{\mathcal{I}}}
{\hat{P}(A),\hat{P}(\alpha)}=\me{A'}{\hat{\mathcal{O}}}
{\hat{P}(A)}\cdot\me{\alpha'}{\hat{\mathcal{I}}}
{\hat{P}(\alpha)}
\ee
with $\hat{P}\in\mathcal{A}$, hence,
\be\la{eq.perma}\hat{P}(A)=T_P^\intercal\,A\,T_P:=A^P\;\;\;.\ee
\be\la{eq.oplist}
\hat{\mathcal{O}}\in\left\lbrace
\mathbb{1}~;~
-\sumin\prodcart\ve{\partial}_c^\intercal(i)\mathbb{1}\ve{\partial}_c(i)~;~
\sumin q(i)L^z(i)~;~
\sumin q(i)(\omega_xx^2(i)+\omega_yy^2(i)+\omega_zz^2(i))~;~
\sumin q(i)\sigma^z(i)~;~
\sumijn e^{-\frac{\Lambda^2}{4}\left[\ve{r}(i)-\ve{r}(j)\right]^2}
\right\rbrace\
\ee

\newpage

\paragraph{The matrix elements}

\be\la{tab.mes}
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{2.4}
\begin{array}{l|c|c}
\hline
\hat{\mathcal{O}} & 
\me{A'}{\hat{\mathcal{O}}}{\hat{P}(A)} &
\me{\alpha'}{\hat{\mathcal{I}}}{\hat{P}(\alpha)} \\
\hline
% identity
\mathbb{1}:=\mathbb{1}_{\ve{r}}^P\cdot\mathbb{1}_{\ve{s}}^P           &
\left(\frac{(2\pi)^{3\texttt{N}}}
{\det\mathbb{A}_x\det\mathbb{A}_y\det\mathbb{A}_z}\right)^\frac{1}{2} &
\sum\limits_{n,n'}^{\texttt{ncmp}(\alpha),\texttt{ncmp}(\alpha')} C^n_\alpha C^{n'}_{\alpha'}
\overlap{\ve{s}^{n'};\ve{t}^{n'}}{\hat{P}(\ve{s}^n);\hat{P}(\ve{t}^n)} \\
% E_kin
-\sumin\prodcart\ve{\partial}_c^\intercal(i)\mathbb{1}\ve{\partial}_c(i)\cdot
\mathbb{1}_{\ve{s}}^P&
\mathbb{1}_{\ve{r}}^P\cdot\prodcart(A_c)_{im}(\mathbb{A}^{-1}_c)_{mn}(A^P_c)_{ni} & \mathbb{1}_{\ve{s}}^P\\
% Lz from background field
\sumin q(i)L_z(i)=q(i)\left[x(i)\partial_{y}(i)-y(i)\partial_{x}(i)\right] &  0 &
\mathbb{Q}^P_{\ve{s}}:=\sumin\sum\limits_{n,n'}^{\texttt{ncmp}(\alpha),\texttt{ncmp}(\alpha')} \left[2-\ve{t}^n(\hat{\small P}(i))\right]~C^n_\alpha C^{n'}_{\alpha'}
\overlap{\ve{s}^{n'};\ve{t}^{n'}}{\hat{P}(\ve{s}^n);\hat{P}(\ve{t}^n)}
\\
% osci pot from background field
\sumin q(i)(\omega_xx^2(i)+\omega_yy^2(i)+\omega_zz^2(i))                &
\mathbb{1}_{\ve{r}}^P\cdot\prodcart\omega_c\sumin(\mathbb{A}_c^{-1})_{ii} & \mathbb{Q}^P_{\ve{s}} \\
% magnetic moment 
\sumin q(i)\sigma^z(i)                                                 &
\mathbb{1}_{\ve{r}}^P &
\sumin\sum\limits_{n,n'}^{\texttt{ncmp}(\alpha),\texttt{ncmp}(\alpha')} \left[2-\ve{t}^n(\hat{\small P}(i))\right]~\left[3-2\ve{s}^n(\hat{\small P}(i))\right]~C^n_\alpha C^{n'}_{\alpha'}
\overlap{\ve{s}^{n'};\ve{t}^{n'}}{\hat{P}(\ve{s}^n);\hat{P}(\ve{t}^n)}  \\
% EFTnoPi LO two-body contact
\sumijn e^{-\frac{\Lambda^2}{4}\left[\ve{r}(i)-\ve{r}(j)\right]^2}                &  \\
% EFTnoPi LO three-body contact
\sumijk e^{-\frac{\Lambda^2}{4}\left[\ve{r}(i)-\ve{r}(j)\right]^2}
e^{-\frac{\Lambda^2}{4}\left[\ve{r}(i)-\ve{r}(k)\right]^2}                &  \\
\hline
\end{array}
\ee
with
\begin{align}
&\mathbb{A}_x=A'_x+A^P_x&\\
&\frac{\Lambda^2}{4}=\texttt{apot}&
\intertext{Spin-isospin wave function for the deuteron:}
&\ket{\alpha\hat{=}{}^3S_1;m_S=\left\lbrace{0\atop\pm 1}\right\rbrace}=\Big[\left\lbrace{\ket{\texttt{12}}+\ket{\texttt{21}}\atop\ket{\texttt{11/22}}+\ket{\texttt{11/22}}}\right\rbrace\Big]\otimes
\Big[\ket{\texttt{pn}}-\ket{\texttt{np}}\Big]&\\
&\ve{s}^n(i)=\left\lbrace{1~\Rightarrow~\sigma^z(i)=\uparrow\atop 2~\Rightarrow~\sigma^z(i)=\downarrow}\right.\;\;\;\text{and}\;\;\;\ve{t}^n(i)=\left\lbrace{1~\Rightarrow~\tau^z(i)=\texttt{p}\atop 2~\Rightarrow~\tau^z(i)=\texttt{n}}\right.&
\intertext{$\texttt{Operators.cpp}$}
\texttt{iop}=0: &\;\;\;\;\; \mathbb{1} & \text{(Wigner)}\\
\texttt{iop}=1: &\;\;\;\;\; -\frac{1}{4}\left[\mathbb{1}+\ve{\tau}(i)\cdot\ve{\tau}(j)\right]
\left[\mathbb{1}+\ve{\sigma}(i)\cdot\ve{\sigma}(j)\right] & \text{(Majorana)}\\
\texttt{iop}=2: &\;\;\;\;\; \frac{1}{2}\left[\mathbb{1}+\ve{\sigma}(i)\cdot\ve{\sigma}(j)\right] &
 \text{(Bartlett)}\\
\texttt{iop}=3: &\;\;\;\;\; -\frac{1}{2}\left[\mathbb{1}+\ve{\tau}(i)\cdot\ve{\tau}(j)\right] &
 \text{(Heisenberg)}
\end{align}

\begin{table}
\begin{equation*}\la{tab.isoOP}
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{2.4}
\begin{array}{l|c|c}
\hline
\texttt{\#} & \texttt{O\red{1}b\_\#} & \texttt{O\red{2}b\_\#} \\
\hline
0 &
\overlap{\ve{\xi}^{n'}_1,\ldots,\ve{\xi}^{n'}_N}
{\ve{\xi}^n_{\pe{1}},\ldots,\ve{\xi}^n_{\pe{N}}} &
 \overlap{\ve{\xi}^{n'}_1,\ldots,\ve{\xi}^{n'}_N}
{\ve{\xi}^n_{\pe{1}},\ldots,\ve{\xi}^n_{\pe{N}}} \\
1 & -- &
\overlap{\ve{\xi}^{n'}_1,\ldots,\ve{\xi}^{n'}_i,\ldots,\ve{\xi}^{n'}_j,\ldots,\ve{\xi}^{n'}_N}
{\ve{\xi}^n_{\pe{1}},\ldots,\ve{\xi}^{n}_{\pe{j}},\ldots,\ve{\xi}^{n}_{\pe{i}},\ldots,\ve{\xi}^n_{\pe{N}}} \\
\end{array}
\end{equation*}
\caption{Two basic operations are performed by the interaction Hamiltinian
Eq.\eqref{eq.ham}, namely, the identity, and the exchange.}
\end{table}
\end{landscape}

\end{document}